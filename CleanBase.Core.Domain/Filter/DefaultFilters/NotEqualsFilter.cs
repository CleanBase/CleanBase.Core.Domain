...
{
    /// <summary>
    /// Represents a filter that checks for inequality between a specified field and a given value.
    /// </summary>
    public class NotEqualsFilter : BaseFilter
    {
        /// <summary>
        ...
        /// <summary>
        /// Builds a LINQ expression to filter entities where the specified field does not equal the given value.
        /// </summary>
        ...
        public override Expression<Func<T, bool>> BuildExpression<T>()
        {
            ...
            // Special handling for case-insensitive string comparison.
            if (member.Type == typeof(string) && Value is string)
            {
                ...
                body = Expression.NotEqual(memberToLower, constantToLower);
            }
            else
            {
